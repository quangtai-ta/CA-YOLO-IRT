...
class C3_CA(C3):
    ...
    def forward(self, x):
        return self.att(super().forward(x))
#############################################################################################
########## C3_SA: C3 module with integrated Spatial Attention
class C3_SA(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = self.sigmoid(self.sa_conv(torch.cat([avg_out, max_out], dim=1)))
        return self.cv3(combined * mask)
...
####################  ISPPF (Improved SPPF) MODULE 

class ISPPF(nn.Module):
    def __init__(self, c1, c2, k=5):
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1) # Conv of YOLOv5 (SiLU)
        
        # Branch Max Pooling
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        
        # Branch Global Average Pooling
        self.gap = nn.AdaptiveAvgPool2d(1)
        
        # input channel is aggregation of 5 channels c_ * 5 
    ...
    def forward(self, x):
        ...
        y2 = self.gap(x).expand_as(x)
        
        # Concat: original + 3 maxpools + 1 global_avg
...
class SPPCSPC(nn.Module):
    # CSP Spatial Pyramid Pooling - Cross Stage Partial
    ...
    def forward(self, x):
        ...
        # Nhanh chinh di qua SPP (Maxpooling song song)
        y1 = torch.cat([x1] + [m(x1) for m in self.m], 1)
        y1 = self.cv6(self.cv5(y1))
        # Nhanh phu (Shortcut)
        y2 = self.cv2(x)
        # Ket hop hai nhanh (CSP)
...
#         self.cv2 = Conv(c_, c_, 5, 1, None, 1, act=act)
...
class CBAM(nn.Module):
    def __init__(self, c1, c2, reduction=16, kernel_size=7):
        super().__init__()
        assert c1 == c2, "CBAM does not change channel size"
...
class GhostConv_S(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1):
        ...
        self.cv2 = Conv(c_, c_out_cv2, 5, 1, None, 1, act=True)

    def forward(self, x):
        y = self.cv1(x)
        return torch.cat((y, self.cv2(y)), 1)

class CA_Ghost(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        super(CA_Ghost, self).__init__()
        ...
        # 
        self.conv1 = GhostConv_S(inp, mip, k=1)
        
        self.conv_h = nn.Sequential(
            nn.Conv2d(mip, inp, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(inp)
        )
